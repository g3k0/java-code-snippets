	...
	private void produce() throws InterruptedException {
		...
		while (bufferCondiviso.size() == SIZE) {
			synchronized (bufferCondiviso) {
				System.out.println("Il buffer � pieno, il thread Producer resta in attesa... la dimensione del buffer adesso �: " + bufferCondiviso.size());
		...
		synchronized (bufferCondiviso) {
			...
			System.out.println("Il thread Producer ha aggiunto al buffer l'elemento: " + i + " la dimensione del buffer adesso �: " + bufferCondiviso.size());
		}
	}
}